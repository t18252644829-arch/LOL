...
```
①  打几局某英雄 → 截图 → 批量识别.bat            得到 _汇总.json
②  把 _汇总.json 拖到 分析简报.bat(填段位/位置/版本)
       简报里含「上次任务进度」(自动核对) + 版本事实 + 补丁说明
③  把简报发我 → 我:核对上次任务 + 结合版本/meta给建议 + 布置新任务
④  我会给你几行 add 命令,粘到 PowerShell 执行即可存下新任务:
       py -3.12 -m lol_coach.tasks add --desc "前10分钟补刀不落后" --champion 诺手 --metric avg_cs_diff --target 0
⑤  带着任务打下一批 → 回到 ①
```
...
```bash
py -3.12 -m lol_coach.tasks list              # 查看(或双击 查看任务.bat)
py -3.12 -m lol_coach.tasks done 1            # 完成第1项
py -3.12 -m lol_coach.tasks drop 2            # 放弃第2项
```

可自动核对的指标:`avg_cs_diff`(补刀差)、`avg_kp`(参团率)、`avg_gold_diff`(经济差)、
`avg_cs`(补刀)。其它(如「5层再交R」)作为自评任务,不绑指标。
